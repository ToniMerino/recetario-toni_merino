...
const CURRENT_DATA_VERSION=window.RTM_DATA_VERSION||"2.1.0";
let adminMode=false;
...
function setAdminMode(value){adminMode=Boolean(value);const badge=$("modeBadge"),btn=$("adminBtn"),notice=$("adminNotice"),add=$("addBtn"),form=$("formView");badge.textContent=adminMode?"Modo administración":"Modo consulta";badge.classList.toggle("admin",adminMode);btn.textContent=adminMode?"🔓 Salir de administración":"🔐 Administración";btn.classList.toggle("active",adminMode);notice.classList.toggle("hidden",!adminMode);add.classList.toggle("hidden",!adminMode);if(!adminMode&&form&&!form.classList.contains("hidden"))show("catalogView");const detail=$("detailView");if(detail&&!detail.classList.contains("hidden")){const id=cookState.recordId||null; /* el detalle se refresca al volver a abrirlo */}}
