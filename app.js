...
const CURRENT_DATA_VERSION=window.RTM_DATA_VERSION||"1.9.0";
...
function loadRecords(){
  ...
  const merged=base.filter(r=>!deletedIds.has(r.id)).map(r=>{
    ...
    return {...r,...local,vegetarian:local.vegetarian??r.vegetarian,vegan:local.vegan??r.vegan,dietary_note:local.dietary_note??r.dietary_note,favorite:local.favorite??r.favorite,image:local.image??r.image};
  });
