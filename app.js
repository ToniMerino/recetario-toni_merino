...
const CURRENT_DATA_VERSION=window.RTM_DATA_VERSION||"2.1.12";
let adminMode=false;
...
function loadRecords(){
  ...
  const merged=base.filter(r=>{
    ...
    // confundirse con una eliminación local. Esto preserva las incorporaciones oficiales en cada actualización de catálogo.
    ...
    return {...r,...local,family:r.family,type:r.type,vegetarian:local.vegetarian??r.vegetarian,vegan:local.vegan??r.vegan,dietary_note:local.dietary_note??r.dietary_note,favorite:local.favorite??r.favorite,image:local.image??r.image,gallery:local.gallery??r.gallery};
  });
...
const RTM_TAXONOMY={
"Masas, panes y bollería":["Pan","Pan dulce","Bollería fermentada","Focaccia","Pizza","Coca salada","Empanada","Empanadilla"],
"Pastelería y repostería":["Bizcocho","Tarta","Galleta","Magdalena","Hojaldre dulce","Coca dulce","Vacherin","Donete","Financier"],
"Postres helados y fríos":["Helado","Sorbete","Granizado","Polo","Postre frío"],
"Postres fríos y de cuchara":["Yogur","Postre lácteo","Mousse dulce","Panna cotta","Flan","Crema dulce"],
"Carnes y aves":["Pollo","Cerdo","Ternera","Cordero","Conejo","Caza","Carne picada","Casquería","Charcutería","Estofado de ternera","Foie gras","Fricandó"],
"Pescados y mariscos":["Pescado","Bacalao","Marisco","Cefalópodo"],
"Arroces":["Paella","Arroz seco","Arroz meloso","Arroz caldoso","Risotto"],
"Pastas y fideos":["Pasta","Lasaña","Canelones","Fideuà"],
"Legumbres":["Garbanzos","Lentejas","Judías","Otras legumbres","Guiso de legumbres"],
"Verduras y hortalizas":["Verduras","Hortalizas","Setas"],
"Sopas, cremas y gazpachos":["Sopa","Escudella","Bisqué","Crema caliente","Crema fría","Gazpacho"],
"Entrantes y aperitivos":["Croqueta","Buñuelo","Ensaladilla","Paté","Aperitivo","Mousse salada","Entrante de verduras y marisco","Falafel"],
"Salsas, aderezos y condimentos":["Salsa","Mayonesa","Vinagreta","Adobo","Aceite aromatizado","Guacamole"],
"Bebidas":["Batido","Zumo","Limonada","Bebida","Bebida vegetal","Bebida tradicional"],
"Técnicas y bases culinarias":["Caldo base","Fondo","Cocción","Sofrito","Masa base","Marca para arroces","Base dulce"]
};
function populateTaxonomyForm(familyValue="",typeValue=""){const f=document.forms.recipeForm;if(!f)return;const fs=f.elements.family,ts=f.elements.type;const families=Object.keys(RTM_TAXONOMY);fs.innerHTML='<option value="">Selecciona familia</option>'+families.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join("");if(families.includes(familyValue))fs.value=familyValue;populateTypeOptions(fs.value,typeValue)}
function populateTypeOptions(family,typeValue=""){const f=document.forms.recipeForm;if(!f)return;const ts=f.elements.type,types=RTM_TAXONOMY[family]||[];ts.innerHTML='<option value="">Selecciona tipo</option>'+types.map(x=>`<option value="${esc(x)}">${esc(x)}</option>`).join("");if(types.includes(typeValue))ts.value=typeValue}
function emptyForm(){const f=$("recipeForm");f.reset();f.elements.id.value="";populateTaxonomyForm();$("formTitle").textContent="Añadir ficha"}
function editRecord(id){if(!requireAdmin())return;const r=records.find(x=>x.id===id);if(!r)return;const f=$("recipeForm");const values={id:r.id,record_type:r.record_type,title:r.title,origin:r.origin,author:r.author,type:r.type,family:r.family,servings:r.servings,time:r.time,difficulty:r.difficulty,status:r.status,ingredients:(r.ingredients||[]).join("\n"),steps:(r.steps||[]).join("\n"),presentation:r.presentation,tips:r.tips,notes:r.notes,tags:(r.tags||[]).join(", "),vegetarian:String(r.vegetarian===true),vegan:String(r.vegan===true),dietary_note:r.dietary_note||""};populateTaxonomyForm(r.family,r.type);Object.entries(values).forEach(([k,v])=>{if(f.elements[k]&&k!=="family"&&k!=="type")f.elements[k].value=v||""});$("formTitle").textContent="Editar ficha";show("formView")}
...
document.forms.recipeForm.elements.family.addEventListener("change",e=>populateTypeOptions(e.target.value));
populateTaxonomyForm();
