...
let records=loadRecords(),activeFilter="all",activeFamily="all",activeFeature="all";const $=id=>document.getElementById(id);
...
function renderCatalog(){const q=$("searchInput").value.trim().toLowerCase();const filtered=records.filter(r=>activeFilter==="all"||r.record_type===activeFilter).filter(r=>activeFamily==="all"||r.family===activeFamily).filter(featureMatches).filter(r=>!q||matches(r,q)).sort((a,b)=>a.title.localeCompare(b.title,"es"));$("count").textContent=`${filtered.length} ${filtered.length===1?"ficha":"fichas"}`;$("catalog").innerHTML=filtered.map(r=>`<article class="card ${r.record_type==="technique"?"technique":""}" data-id="${esc(r.id)}"><div class="card-bar"></div><div class="card-image">${r.favorite?`<span class="card-favorite" title="Favorita" aria-label="Favorita">★</span>`:""}${esc(r.emoji||(r.record_type==="technique"?"🥣":"🍽️"))}</div><div class="card-body"><span class="kind">${r.record_type==="technique"?"Técnica culinaria":"Receta"}</span><h2>${esc(r.title)}</h2><p>${esc(r.family||r.origin||"")}</p></div></article>`).join("")||`<p class="count">No se han encontrado fichas.</p>`;document.querySelectorAll(".card").forEach(c=>c.addEventListener("click",()=>openRecord(c.dataset.id)))}
function quick(l,v){return `<div><b>${l}</b>${esc(v||"No indicado")}</div>`}
function openRecord(id){const r=records.find(x=>x.id===id);if(!r)return;$("detail").innerHTML=`<header class="detail-head ${r.record_type==="technique"?"technique":""}"><div class="detail-head-top"><span class="kind">${r.record_type==="technique"?"Técnica culinaria":"Receta"}</span><button class="favorite-btn detail-favorite ${r.favorite?"active":""}" onclick="toggleFavorite('${esc(r.id)}')" aria-pressed="${r.favorite?"true":"false"}">${r.favorite?"★ Favorita":"☆ Marcar favorita"}</button></div><h1>${esc(r.title)}</h1><p class="detail-sub">${esc([r.author,r.origin].filter(Boolean).join(" · "))}</p></header><div class="detail-body"><div class="quick">${quick("Tipo",r.type)}${quick("Familia culinaria",r.family)}${quick("Raciones",r.servings)}${quick("Tiempo",r.time)}${quick("Dificultad",r.difficulty)}</div><div class="dietary">${r.vegetarian?`<span class="diet-badge">Vegetariana</span>`:""}${r.vegan?`<span class="diet-badge">Vegana</span>`:""}${r.dietary_note?`<p>${esc(r.dietary_note)}</p>`:""}</div><h2>Ingredientes</h2><ul>${(r.ingredients||[]).map(x=>`<li>${esc(x)}</li>`).join("")}</ul><h2>Preparación</h2><ol>${(r.steps||[]).map(x=>`<li>${esc(x)}</li>`).join("")}</ol>${r.presentation?`<h2>Presentación</h2><p>${esc(r.presentation)}</p>`:""}${r.tips?`<h2>Consejos</h2><div class="note">${esc(r.tips)}</div>`:""}${r.status||r.notes?`<h2>Mi valoración</h2><p><b>Estado:</b> ${esc(r.status||"—")}</p>${r.notes?`<p>${esc(r.notes)}</p>`:""}`:""}<div class="tags">${(r.tags||[]).map(t=>`<span class="tag">#${esc(t.replace(/^#/,""))}</span>`).join("")}</div><div class="detail-actions"><button class="primary" onclick="editRecord('${esc(r.id)}')">Editar</button><button onclick="window.print()">Guardar PDF</button><button onclick="deleteRecord('${esc(r.id)}')">Eliminar</button></div></div>`;show("detailView")}
